...
# View trace plots of each parameter
traceplot(fit1)
...
# convert factor of subject ids to integers
...
# Note: random effects increase processing time exponentially, see: nested for loops
...
propInRope(m2_g2s_withpriors,ropemin = -1, ropemax = 1)
...
m3_g2s <- myglmer2stan( Reaction ~ Days*Days2 + (Days | subject_index), data=sleepstudy,
                        ...
                        warmup=100, #burn-in period
                        iter = 1000, # number of steps per chain
                        chains=4 # when this is NULL the model is built and run like normal
) 

stanmer(m3_g2s)
mypostplot(m3_g2s,ropemin = -1,ropemax = 1)
